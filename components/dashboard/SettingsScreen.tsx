...
const SettingsScreen: React.FC<SettingsScreenProps> = ({ locations = [], onAddLocation, onUpdateLocation, onDeleteLocation, currentUser }) => {
    ...
    const handleSaveFleetItem = async (e: React.FormEvent) => {
        ...
        if (error) {
            showError('Erro ao salvar Jet Ski: ' + error.message);
        } else {
                        ...
                        <div className="animate-[fade-in-up_0.3s_ease-out] w-full">
                             <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                                ...
                                <button 
                                    ...
                                    className="px-6 py-2.5 bg-primary text-white rounded-lg text-sm font-bold shadow-md shadow-primary/20 hover:bg-primary/90 transition-all active:scale-95 flex items-center gap-2 w-full md:w-auto justify-center"
                                >
                             ...
                             <div className="overflow-x-auto rounded-lg border border-gray-200">
                                <table className="w-full text-left border-collapse">
                                    <thead className="bg-white border-b border-gray-200">
                                        <tr>
                                            <th className="py-4 px-6 text-xs font-bold uppercase tracking-wider text-primary/80 whitespace-nowrap">Modelo</th>
                                    ...
                                    <tbody className="divide-y divide-gray-100">
                                        {fleet.map(item => (
                                            <tr key={item.id} className="hover:bg-blue-50/30 transition-colors">
                                                ...
                                                <td className="py-4 px-6 whitespace-nowrap">
                                                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold border ${item.status === 'Disponível' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : (item.status === 'Manutenção' ? 'bg-amber-50 text-amber-700 border-amber-100' : 'bg-red-50 text-red-700 border-red-100')}`}>
                                                        <span className={`size-1.5 rounded-full ${item.status === 'Disponível' ? 'bg-emerald-500' : (item.status === 'Manutenção' ? 'bg-amber-500' : 'bg-red-500')}`}></span>
                                                        {item.status}
                                                    </span>
                                                </td>
                                                <td className="py-4 px-6 whitespace-nowrap text-right">
                                                    <div className="flex items-center justify-end gap-2">
                                                        ...
                                                        <button onClick={() => handleDeleteFleetItem(item.id)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="Excluir">
                                        ...
                                        {fleet.length === 0 && (
                                            <tr>
                                                <td colSpan={5} className="py-12 text-center text-gray-500 text-sm">Nenhum equipamento cadastrado.</td>
                                            </tr>
                                        )}
                                    </tbody>
                                </table>
                    ...
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-[fade-in-up_0.2s_ease-out]" onClick={e => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleSaveFleetItem} className="p-6 flex flex-col gap-4">
                            ...
                            <div>
                                ...
                                <input value={fleetPlate} onChange={e => setFleetPlate(e.target.value)} className="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:border-primary outline-none focus:ring-1 focus:ring-primary/20 transition-all" placeholder="Ex: JMS-01" required />
                            </div>
                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-1">Status</label>
                                <div className="relative">
                                    <select value={fleetStatus} onChange={e => setFleetStatus(e.target.value as any)} className="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:border-primary outline-none focus:ring-1 focus:ring-primary/20 transition-all appearance-none">
